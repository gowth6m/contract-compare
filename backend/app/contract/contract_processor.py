...
class ContractProcessor:
    ...
    @staticmethod
    def extract_key_properties(clauses: List[Clause]) -> dict:
        properties = {
            "duration": next(
                (
                    clause.content
                    for clause in clauses
                    if "year" in clause.content.lower()
                ),
                "Not Found",
            ),
            "indemnity": any(
                "indemnity" in clause.content.lower() for clause in clauses
            ),
            "discount": next(
                (clause.content for clause in clauses if "%" in clause.content),
                "Not Found",
            ),
        }
        return properties
