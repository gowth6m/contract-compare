...
class SQSSecrets(BaseModel):
    UPLOADED_FILES: str


def get_sqs_secrets() -> SQSSecrets:
    secrets_manager = get_secrets_manager()
    try:
        base_secrets = secrets_manager.get_secret("backend/sqs")
        base_secrets_obj = json.loads(base_secrets)
        return SQSSecrets(**base_secrets_obj)
    except (json.JSONDecodeError, ValidationError) as e:
        raise HTTPException(
            status_code=500, detail=f"Error parsing or validating secret data: {str(e)}"
        )
...
def get_s3_secrets() -> S3Secrets:
    secrets_manager = get_secrets_manager()
    try:
        base_secrets = secrets_manager.get_secret("backend/s3")
        base_secrets_obj = json.loads(base_secrets)
        return S3Secrets(**base_secrets_obj)
    except (json.JSONDecodeError, ValidationError) as e:
        raise HTTPException(
            status_code=500, detail=f"Error parsing or validating secret data: {str(e)}"
        )
...
class WebSocketSecrets(BaseModel):
    ENDPOINT: str


def get_websocket_secrets() -> WebSocketSecrets:
    secrets_manager = get_secrets_manager()
    try:
        base_secrets = secrets_manager.get_secret("backend/websocket")
        base_secrets_obj = json.loads(base_secrets)
        return WebSocketSecrets(**base_secrets_obj)
    except (json.JSONDecodeError, ValidationError) as e:
        raise HTTPException(
            status_code=500, detail=f"Error parsing or validating secret data: {str(e)}"
        )
